...
class NginxAPITester:
    def __init__(self):
        ...
        # Common Nginx endpoints and paths to test
        self.endpoints = [
            "",  # Root path
            "nginx_status",
            "server-status",
        ...
        try:
            if method.upper() == "GET":
                response = requests.get(
                    ...
                    timeout=5,
                    verify=False,
                    allow_redirects=False  # Don't follow redirects to better identify responses
                )
            else:
            ...
            if 'nginx' in server_header.lower():
                print(f"[{response.status_code}] {method} {full_url} - Nginx Detected: {server_header}")
            else:
    ...
    def test_nginx_config(self, url):
        """Test for common Nginx misconfigurations"""
        tests = [
            "/.git/config",  # Git exposure
            "/nginx.conf",   # Config file exposure
...
def main():
    ...
    target_url = input("Masukan target Nginx URL (e.g., https://example.com): ").strip()
    ...
    try:
        ...
        tester = NginxAPITester()
        print("[INFO] Testing common Nginx configurations...")
        tester.test_nginx_config(target_url)
